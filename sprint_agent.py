...
"""
Sprint Status Agent — Bridges to an existing Azure AI Foundry agent.

Instead of running its own LLM, this agent forwards messages to a Foundry agent
that already has instructions, model config, and WorkIQ connections for mining
M365 data (emails, meetings, chats, files) to produce engineering status reports.
...
class SprintStatusAgent(AgentInterface):
    """
    A365-hosted agent that delegates to an existing Foundry agent.

    The Foundry agent handles sprint intelligence (instructions, WorkIQ for M365 data).
    ...
    def _get_client_for_user(self, user_token: Optional[str] = None) -> OpenAI:
        ...
        passthrough to Work IQ), or the default API key client.
        """
        if user_token:
            return OpenAI(
                api_key="unused",
    ...
    async def process_user_message(
        ...
        with span_ctx:
            try:
                # Exchange user token for OBO access (Work IQ)
    ...
    async def handle_agent_notification_activity(
        ...
        """Handle A365 notifications by forwarding to the Foundry agent for sprint-aware processing."""
        try:
            ...
            if notification_type == NotificationTypes.EMAIL_NOTIFICATION:
                ...
                message = (
                    "You received the following email that may contain sprint-relevant information "
                    "(status updates, blockers, decisions, or action items). "
                    "Please analyze it and extract any engineering status signals.\n\n"
                    f"{email_body}"
                )
            elif notification_type == NotificationTypes.WPX_COMMENT:
                comment_text = notification_activity.text or ""
                message = (
                    f"You were mentioned in a document comment: {comment_text}\n"
                    "Please review — this may contain a status update, blocker, or action item."
                )
            else:
    ...
    # =========================================================================
    # OBO TOKEN EXCHANGE (Work IQ requires user delegation)
    # =========================================================================
    ...
    async def _exchange_user_token(
        ...
        allows Work IQ to access M365 data on behalf of the agent's A365 identity.
        """
        import jwt as pyjwt
        ...
        logger.warning("⚠️ No user token available — calling Foundry with API key (WorkIQ will fail)")
        return None
    ...
    def _call_foundry(self, user_id: str, message: str, user_token: Optional[str] = None) -> str:
        ...
        # Use user-authenticated client for OBO passthrough (Work IQ)
